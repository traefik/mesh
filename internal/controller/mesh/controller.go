...
func NewMeshController(clients *k8s.ClientWrapper) *Controller {
	...
	nameTrafficTarget, lwTrafficTarget, objTrafficTarget := newTrafficTargetWatch(clients)
	nameHTTPGroup, lwHTTPGroup, objHTTPGroup := newHTTPRouteGroupWatch(clients)
	nameTrafficSplit, lwTrafficSplit, objTrafficSplit := newTrafficSplitWatch(clients)
